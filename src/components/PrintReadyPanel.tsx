...
function DownloadBtn({ url, label, primary }: { url: string; label: string; primary?: boolean }) {
  return (
...
export function PrintReadyPanel({
  readiness, parts, canPrepare, preparing, onPrepare,
}: {
        ...
        <>
          ...
          <div style={{ marginTop: 14, display: "flex", flexWrap: "wrap", gap: 8 }}>
            {readiness.formats.map((f) => (
              <DownloadBtn key={f.format} url={f.url} label={f.label} primary={f.format === "3mf"} />
            ))}
          </div>
      ...
      {hasParts && (
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${LAND.border}` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {parts!.map((p, i) => (
              <a key={i} href={p.meshUrl} download style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, color: LAND.ink2, textDecoration: "none", borderBottom: `1px solid ${LAND.border}`, paddingBottom: 5 }}>
                <span>{p.name}</span>
