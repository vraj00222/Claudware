...
## ARIZE AX — LLM observability (sponsor #8) — merged to main (PR #31)
- [x] Auto-instrumented Anthropic SDK (every messages.create → LLM span with prompts/responses/tokens)
- [x] Top-level CHAIN span wraps each generation pipeline (engine, duration, cache hit/miss, had-repair)
- [x] LLM-as-judge evaluator (Haiku scores relevance/printability/completeness → span attributes)
- [x] /api/arize status + test-span endpoint; key-gated no-op (zero overhead when keys absent)
- [x] OpenTelemetry → OTLP/proto → otlp.arize.com; project "claudware"
- NEXT: Verify traces visible in Arize UI with judges at booth.
